...
::: {.alert .alert-warning}
<strong>Note:</strong> this notebook uses synthetic data for demonstration purposes. Therefore, this script can not be used to answer the research stated above.
:::
...
```{r, include=TRUE, echo=TRUE, warning=FALSE, output=FALSE}
...
It is based on the study described in the introduction and has the same structure as a survey export read with `read_appkit_surveys()`.

```{r}
?appkit_survey_entry
```

The data contain an entry survey, the momentary assessments and an exit survey:
...
The first few variables contain metadata on the participant and their device, e.g.:
